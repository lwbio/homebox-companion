...
class LocationView(BaseModel):
    ...
    def url(self) -> str:
        ...
        return f"{settings.effective_link_base_url}/items?location_id={self.id}"
...
class CompactTagView(BaseModel):
    ...
    def url(self) -> str:
        ...
        return f"{settings.effective_link_base_url}/items?tag={self.id}"
...
class CompactItemView(BaseModel):
    ...
    def url(self) -> str:
        ...
        return f"{settings.effective_link_base_url}/items/{self.id}"
...
def add_tree_urls(node: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_location:
        node["url"] = f"{settings.effective_link_base_url}/items?location_id={node.get('id')}"
    else:  # item
        node["url"] = f"{settings.effective_link_base_url}/items/{node.get('id')}"
